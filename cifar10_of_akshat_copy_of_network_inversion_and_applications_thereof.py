# -*- coding: utf-8 -*-
"""cifar10 of Akshat_Copy_of_Network_Inversion_and_Applications_Thereof.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/16OvmNUEhQP5Z0q8ylHRNaQf6hiWjJYOp
"""
...
train_data_dir = os.path.join(cifar_root, 'train')
test_data_dir = os.path.join(cifar_root, 'test')
...
train_set = datasets.CIFAR10(root=cifar_root, train=True, download=True)
test_set = datasets.CIFAR10(root=cifar_root, train=False, download=True)
...
# Download CIFAR-10 from a Google Drive mirror
!gdown https://drive.google.com/uc?id=1viH-82Aw4A7ZxD5Vbmmn9WBdZWQVQSts
...
# Transformation to convert images to PIL format (CIFAR10 images are already in PIL format by default)
transform = transforms.Compose([transforms.ToPILImage()])

# Load the dataset without downloading again
train_set = datasets.CIFAR10(root=cifar10_root, train=True, download=True, transform=None)
test_set = datasets.CIFAR10(root=cifar10_root, train=False, download=True, transform=None)
...
print("CIFAR-10 dataset loaded and data loaders created.")
...
# Define paths for CIFAR-10 dataset
cifar_root = './data/CIFAR10'
train_data_dir = os.path.join(cifar_root, 'train')

# Load the CIFAR-10 dataset to get class names
train_set = datasets.CIFAR10(root=cifar_root, train=True, download=True)
classes = train_set.classes  # This will give you the list of class names
...
for class_name in classes:
    ...
    for image_file in os.listdir(class_path)[:10]:  # Only take the first 10 images
...
for i, class_name in enumerate(classes):
    for j in range(10):
        axs[i, j].imshow(class_images[class_name][j])
        axs[i, j].axis("off")
...
class Classifier(nn.Module):
    def __init__(self, nc, ncf):
        ...
        self.main = nn.Sequential(
            # First convolutional layer, expect 3 channels (for CIFAR-10)
            nn.Conv2d(nc, ncf, 4, 2, 1, bias=False),  # Output: (ncf, 16, 16)
            nn.BatchNorm2d(ncf),
            nn.LeakyReLU(0.2, inplace=True),

            # Second convolutional layer
            nn.Conv2d(ncf, ncf * 2, 4, 2, 1, bias=False),  # Output: (ncf*2, 8, 8)
            nn.BatchNorm2d(ncf * 2),
            nn.LeakyReLU(0.2, inplace=True),

            # Third convolutional layer
            nn.Conv2d(ncf * 2, ncf * 4, 4, 2, 1, bias=False),  # Output: (ncf*4, 4, 4)
            nn.BatchNorm2d(ncf * 4),
            nn.LeakyReLU(0.2, inplace=True),

            # Flatten and pass through fully connected layers
            nn.Flatten(),  # Output: (ncf*4 * 4 * 4)
            nn.Linear(ncf * 4 * 4 * 4, ncf * 2),
            nn.ReLU(),
...
class Generator(nn.Module):
    def __init__(self, nz, ngf, nc, n_classes):
        ...
        # Linear layer to map the class vector to the latent space size
        self.embed = nn.Linear(n_classes, nz)

        # The main model architecture tailored for CIFAR-10 (32x32 images)
        self.main = nn.Sequential(
            nn.ConvTranspose2d(nz * 2, ngf * 8, 4, 1, 0, bias=False),  # Output: (ngf*8, 4, 4)
            nn.BatchNorm2d(ngf * 8),
            nn.ReLU(True),

            nn.ConvTranspose2d(ngf * 8, ngf * 4, 4, 2, 1, bias=False),  # Output: (ngf*4, 8, 8)
            nn.BatchNorm2d(ngf * 4),
            nn.ReLU(True),

            nn.ConvTranspose2d(ngf * 4, ngf * 2, 4, 2, 1, bias=False),  # Output: (ngf*2, 16, 16)
            nn.BatchNorm2d(ngf * 2),
            nn.ReLU(True),

            nn.ConvTranspose2d(ngf * 2, ngf, 4, 2, 1, bias=False),  # Output: (ngf, 32, 32)
            nn.BatchNorm2d(ngf),
            nn.ReLU(True),

            nn.ConvTranspose2d(ngf, nc, 3, 1, 1, bias=False),  # Output: (nc, 32, 32)
            nn.Tanh()
        )
...
if best_model_state is not None:
    model_save_path = os.path.join(save_dir, 'cifar10_csf_10.pth')
...
# Create output directory if it does not exist
output_dir = 'MNIST-Inversion/new2/'
os.makedirs(output_dir, exist_ok=True)
num_epochs = 2000
...
# Create output directory if it does not exist
output_dir = 'MNIST-Inversion/new3/'
os.makedirs(output_dir, exist_ok=True)
num_epochs = 2000
...
# Create output directory if it does not exist
output_dir = 'MNIST-Inversion/new4/'
os.makedirs(output_dir, exist_ok=True)
num_epochs = 2000
